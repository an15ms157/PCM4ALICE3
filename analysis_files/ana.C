...
void ana()
{
  ...
  */
  ...
  TH2D hRapidityPt_Gamma_All{"hRapidityPt_Gamma_All", "Rapidity and Pt All photons", 10000, -5., 5, 10000, 0., 10.};
  TH2D hRapidityPt_Gamma_Detected{"hRapidityPt_Gamma_Detected", "Rapidity and Pt All photons detected", 10000, -5., 5, 10000, 0., 10.};
  TH2D hRapidityPt_Gamma_Decay{"hRapidityPt_Gamma_Decay", "Rapidity and Pt photons from meson decay  ", 10000, -5., 5, 10000, 0., 10.};
  ...
  for (int iEvent = 0; iEvent < nEvents; ++iEvent)
  {
    ...
    for (const auto track : *mcTracks)
    {
      ...
      if (TMath::Abs(track.GetRapidity()) > 1.3)
        continue;
      ...
      if (track.isPrimary())
      {
        ...
        if (track.GetPdgCode() == 211)
          pip_prim.emplace_back(track);
        if (track.GetPdgCode() == -211)
          pim_prim.emplace_back(track);

        if (track.GetPdgCode() == 2212)
          prop_prim.emplace_back(track);
        if (track.GetPdgCode() == -2212)
          prom_prim.emplace_back(track);

        if (track.GetPdgCode() == 321)
          kap_prim.emplace_back(track);
        if (track.GetPdgCode() == -321)
          kam_prim.emplace_back(track);
        ...
        if (track.GetPdgCode() == 22)
        {
          ...
          hRapidityPt_Gamma_All.Fill(track.GetRapidity(), track.GetPt());

          auto mTrackMother_gam0_B = (*mcTracks)[track.getMotherTrackId()];
          if (mTrackMother_gam0_B.GetPdgCode() == 111)
          {
            hPrimPhoton_from_Pi0_B_Pt.Fill(track.GetPt());
            hRapidityPt_Gamma_Decay.Fill(track.GetRapidity(), track.GetPt());
          }
          if (mTrackMother_gam0_B.GetPdgCode() == 221)
          {
            hPrimPhoton_from_Eta_B_Pt.Fill(track.GetPt());
            hRapidityPt_Gamma_Decay.Fill(track.GetRapidity(), track.GetPt());
          }
          if (mTrackMother_gam0_B.GetPdgCode() == 223)
          {
            hPrimPhoton_from_Omega_B_Pt.Fill(track.GetPt());
            hRapidityPt_Gamma_Decay.Fill(track.GetRapidity(), track.GetPt());
          }
          if (mTrackMother_gam0_B.GetPdgCode() == 331)
          {
            hPrimPhoton_from_EtaPrime_B_Pt.Fill(track.GetPt());
            hRapidityPt_Gamma_Decay.Fill(track.GetRapidity(), track.GetPt());
          }
          if (mTrackMother_gam0_B.GetPdgCode() == 113)
          {
            hPrimPhoton_from_Rho0_B_Pt.Fill(track.GetPt());
            hRapidityPt_Gamma_Decay.Fill(track.GetRapidity(), track.GetPt());
          }
          if (mTrackMother_gam0_B.GetPdgCode() == 213)
          {
            hPrimPhoton_from_RhoPlus_B_Pt.Fill(track.GetPt());
            hRapidityPt_Gamma_Decay.Fill(track.GetRapidity(), track.GetPt());
          }
          if (mTrackMother_gam0_B.GetPdgCode() == -213)
          {
            hPrimPhoton_from_RhoMinus_B_Pt.Fill(track.GetPt());
            hRapidityPt_Gamma_Decay.Fill(track.GetRapidity(), track.GetPt());
          }
          if (mTrackMother_gam0_B.GetPdgCode() == 333)
          {
            hPrimPhoton_from_Phi_B_Pt.Fill(track.GetPt());
            hRapidityPt_Gamma_Decay.Fill(track.GetRapidity(), track.GetPt());
          }
          if (mTrackMother_gam0_B.GetPdgCode() == 2114)
          {
            hPrimPhoton_from_Delta0_B_Pt.Fill(track.GetPt());
            hRapidityPt_Gamma_Decay.Fill(track.GetRapidity(), track.GetPt());
          }
          if (mTrackMother_gam0_B.GetPdgCode() == 2214)
          {
            hPrimPhoton_from_DeltaPlus_B_Pt.Fill(track.GetPt());
            hRapidityPt_Gamma_Decay.Fill(track.GetRapidity(), track.GetPt());
          }
          if (mTrackMother_gam0_B.GetPdgCode() == 3212)
          {
            hPrimPhoton_from_Sigma0_B_Pt.Fill(track.GetPt());
            hRapidityPt_Gamma_Decay.Fill(track.GetRapidity(), track.GetPt());
          }
          if (mTrackMother_gam0_B.GetPdgCode() == 310)
          {
            hPrimPhoton_from_KZeroShort_B_Pt.Fill(track.GetPt());
            hRapidityPt_Gamma_Decay.Fill(track.GetRapidity(), track.GetPt());
          }
          if (mTrackMother_gam0_B.GetPdgCode() == 130)
          {
            hPrimPhoton_from_KZeroLong_B_Pt.Fill(track.GetPt());
            hRapidityPt_Gamma_Decay.Fill(track.GetRapidity(), track.GetPt());
          }
          if (mTrackMother_gam0_B.GetPdgCode() == 3122)
          {
            hPrimPhoton_from_Lambda_B_Pt.Fill(track.GetPt());
            hRapidityPt_Gamma_Decay.Fill(track.GetRapidity(), track.GetPt());
          }

          if ((gRandom->Uniform() < (convProb)) && (track.GetPdgCode() == 22))
            gamB_conv.emplace_back(track);
          if ((gRandom->Uniform() < (convProb * eff)) && (track.GetPdgCode() == 22))
          {
            gamma_B_conv_rec_SmearedP.emplace_back(make_pair(track, LVgamB_Smear_P));
            hRapidityPt_Gamma_Detected.Fill(track.GetRapidity(), track.GetPt());
          }
        }
        // continue;
      }
      ...
      if (track.GetPt() < minPt)
        continue;
      // AM- I am interested in conversions all the way
      if (sqrt(track.GetStartVertexCoordinatesX() * track.GetStartVertexCoordinatesX() + track.GetStartVertexCoordinatesY() * track.GetStartVertexCoordinatesY()) > maxR)
        continue;
    ...
    { // gam1 is a pair and not a track
    ...
    for (const auto track : *mcTracks)
    {
      if ((TMath::Abs(track.GetRapidity()) < 1.75) || (TMath::Abs(track.GetRapidity()) > 4.))
        continue;
      ...
      if (track.isPrimary())
      {
        ...
        if (track.GetPdgCode() == 22)
        {
          ...
          if (mTrackMother_gam0_F.GetPdgCode() == 111)
          {
            hPrimPhoton_from_Pi0_F_Pt.Fill(track.GetPt());
          }
          if (mTrackMother_gam0_F.GetPdgCode() == 221)
          {
            hPrimPhoton_from_Eta_F_Pt.Fill(track.GetPt());
          }
          if (mTrackMother_gam0_F.GetPdgCode() == 223)
          {
            hPrimPhoton_from_Omega_F_Pt.Fill(track.GetPt());
          }
          if (mTrackMother_gam0_F.GetPdgCode() == 331)
          {
            hPrimPhoton_from_EtaPrime_F_Pt.Fill(track.GetPt());
          }
          if (mTrackMother_gam0_F.GetPdgCode() == 113)
          {
            hPrimPhoton_from_Rho0_F_Pt.Fill(track.GetPt());
          }
          if (mTrackMother_gam0_F.GetPdgCode() == 213)
          {
            hPrimPhoton_from_RhoPlus_F_Pt.Fill(track.GetPt());
          }
          if (mTrackMother_gam0_F.GetPdgCode() == -213)
          {
            hPrimPhoton_from_RhoMinus_F_Pt.Fill(track.GetPt());
          }
          if (mTrackMother_gam0_F.GetPdgCode() == 333)
          {
            hPrimPhoton_from_Phi_F_Pt.Fill(track.GetPt());
          }
          if (mTrackMother_gam0_F.GetPdgCode() == 2114)
          {
            hPrimPhoton_from_Delta0_F_Pt.Fill(track.GetPt());
          }
          if (mTrackMother_gam0_F.GetPdgCode() == 2214)
          {
            hPrimPhoton_from_DeltaPlus_F_Pt.Fill(track.GetPt());
          }
          if (mTrackMother_gam0_F.GetPdgCode() == 3212)
          {
            hPrimPhoton_from_Sigma0_F_Pt.Fill(track.GetPt());
          }
          if (mTrackMother_gam0_F.GetPdgCode() == 310)
          {
            hPrimPhoton_from_KZeroShort_F_Pt.Fill(track.GetPt());
          }
          if (mTrackMother_gam0_F.GetPdgCode() == 130)
          {
            hPrimPhoton_from_KZeroLong_F_Pt.Fill(track.GetPt());
          }
          if (mTrackMother_gam0_F.GetPdgCode() == 3122)
          {
            hPrimPhoton_from_Lambda_F_Pt.Fill(track.GetPt());
          }

          if ((gRandom->Uniform() < (convProb)) && (track.GetPdgCode() == 22))
            gamF_conv.emplace_back(track);
          if ((gRandom->Uniform() < (convProb * eff)) && (track.GetPdgCode() == 22))
            gamma_F_conv_rec_SmearedP.emplace_back(make_pair(track, LVgamF_Smear_P));
        }
      }
      // Conversions in forward
      if (TMath::Abs(track.GetPdgCode()) != 11)
        continue;
      if (track.GetP() < minPFG)
        continue;
      //// Under construction
      // AM- I am interested in conversions in forward direction
      if (TMath::Abs(track.GetStartVertexCoordinatesZ()) > maxZConv)
        continue;
      ...
      ++nConvF;
      if (track.GetPdgCode() == 11)
        ep_F.emplace_back(track);
      else
        em_F.emplace_back(track);
