...
TString input_file_loc="/home/abhishek/PhD/Work/work_A/photons/output/all_events/PbPb/ana_pTcut_withft3.root";
...
TH1F* fHistoSignalInvMassPtBin_FIT_sigma = new TH1F("fHistoSignalInvMassPtBin_FIT_sigma","fHistoSignalInvMassPtBin_FIT_sigma",nbinspt_proj_fine,pt_bin_proj_fine);
...
for(Int_t iPt=fStartPtBin;iPt<fNBinsPt;iPt++){ 
      ...
      fHistoSignalInvMassPtBin_FIT_mu->SetBinError(iPt,fFitReco->GetParError(1) );

      fHistoSignalInvMassPtBin_FIT_sigma->SetBinContent(iPt,fFitReco->GetParameter(2));
      fHistoSignalInvMassPtBin_FIT_sigma->SetBinError(iPt,fFitReco->GetParError(2) );
...
DrawGammaCanvasSettings( c_meson_FIT_mu, 0.1, 0.05, 0.1, 0.1);
...
if(SWITCH_meson_FIT_sigma){ 

TCanvas *c_meson_FIT_sigma = new TCanvas("c_meson_FIT_sigma","c_meson_FIT_sigma",600,300); 
DrawGammaCanvasSettings( c_meson_FIT_sigma, 0.1, 0.05, 0.1, 0.1);
//c_meson_FIT_sigma->SetLogy();

SetStyleHistoTH1ForGraphs(fHistoSignalInvMassPtBin_FIT_sigma, p_T.Data() , "M_{#gamma #gamma}(GeV/c^{2})", TextSize_lable+0.02, TextSize_title+0.02, TextSize_lable+0.02,TextSize_title+0.02);

fHistoSignalInvMassPtBin_FIT_sigma->Draw("E1");
ltb_general_Additional_text->Draw("SAME");
c_meson_FIT_sigma->SaveAs("./meson_FIT_sigma.png");
