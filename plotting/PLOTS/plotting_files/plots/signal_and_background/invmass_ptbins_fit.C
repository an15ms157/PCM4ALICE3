...
#include "../headers/invmass_ptbins_fit.h"
...
gStyle->SetOptFit(0011); // This is for the mean and sigma 
...
TString fRap                               = "F"; // F or B
fMesonId                                   = 111; // 111 or 221


//PLOTTING SWITCHES 
bool SWITCH_write_to_file               = false;
bool SWITCH_c_meson_signalVSbackground  = false;
bool SWITCH_c_BackgroundPerEvent        = false;
bool SWITCH_c_mesonbackground           = false;
...
void FitSubtractedPureGaussianInvMassInPtBins(TH1D* histoMappingSignalInvMassPtBinSingle, Int_t ptBin ){
    ...
    fFitReco= NULL;
    fFitReco = new TF1("GaussBGLinear","gaus(0)",fMesonFitRange[0],fMesonFitRange[1]); // gaus(0) is a substitute for [0]*exp(-0.5*((x-[1])/[2])**2) and (0) means start numbering parameters at 0
    //fFitLinearBck = NULL;
    ...
    histoMappingSignalInvMassPtBinSingle->Fit(fFitReco,"QRME"); //Quiet mode , fitting range specified , IMPROVE algorithm, parameter errors estimation using the Minos technique 
    fFitReco->SetLineColor(5);
    fFitReco->SetLineWidth(1);
    fFitReco->SetLineStyle(1);
    fFitReco->SetNpx(10000);

    double p0 = fFitReco->GetParameter(0);
    double e0 = fFitReco->GetParError(0);
    double p1 = fFitReco->GetParameter(1);
    double e1 = fFitReco->GetParError(1);
    double p2 = fFitReco->GetParameter(2);
    double e2 = fFitReco->GetParError(2);
